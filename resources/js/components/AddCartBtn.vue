                ...
                <b-modal :id="'modal-submenu-'+product_id" hide-footer no-stacking>
                    ...
                    <div class="d-block text-center">
                        ...
                        <b-form-checkbox-group :id="'modal-submenu-check-'+product_id" v-model="selected"
                            ...
                            <li class="sub-item" v-for="sub in getFoodSub()" v-bind:key="sub">
...
<script>
export default {
    props: ["product_id", "product_data","rest"],
    data() {
        return {
            selected: null,
            weight:100,
    ...
    methods: {
        ...
        addToCartWithSub() {
            ...
            this.selected.forEach((item, key) => {
                if (key <= this.selected.length - 3) {
                    this.incProduct();
                }
            });
            this.tmp = this.selected.join();
            this.incProduct();
            this.$store.dispatch("addSub", {id: this.product_id, more_info: this.tmp})
        ...
        getDayOfWeek() {
            ...
            const number = myDate.getUTCDay();
            const time = myDate.getUTCHours();
            if (this.rest.name === "Лабиринт" && number === 5 || number === 6){
                this.sendMessage("К сожалению, в " + this.rest.name + "по выходным дням заказы не оформляются ")
                return true;
            }
            else {
                return false;
